class Spree::Admin::AvailableSpacesController <  Spree::Admin::BaseController
  ...
  def index 
      ...
      @bookings = Spree::Booking.find_by_sql "SELECT  filter_date.id::date as day,coalesce(sum(spree_bookings.id),0) as total_space FROM  spree_bookings  RIGHT JOIN ( select '#{start_date}'::date + cast(numlist.id::text || ' days' as interval) as  id  from (SELECT n1.col_i + n10.col_i*10 + n100.col_i*100 AS id  FROM nums n1 cross join nums as n10 cross join nums as n100) as numlist where (select '#{start_date}'::date + cast(numlist.id::text || ' days' as interval)) <= '#{end_date}'::date ) filter_date ON (spree_bookings.pickup_date::date between filter_date.id and spree_bookings.delivery_date::date) GROUP BY filter_date.id" 
